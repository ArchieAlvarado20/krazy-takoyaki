    ...
    if(is_array($obj))
    {
        if($obj['data_type'] == "search")
        {
            ...
            if($obj['text']){
                ...
                $barcode = $obj['text'];
                $text = "%".$obj['text']."%";
                $qry = "select * from tblproduct where category = '1' && description like :find || p_name = :p_name order by p_name asc";
                $row = $productClass->query($qry,['find'=>$text,'p_name'=>$barcode]); 
            }else{
                //get all
                $row = $productClass->query("select * from tblproduct where category = '1' order by p_name asc");
            }
