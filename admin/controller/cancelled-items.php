...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if($cashier1){
        $sales = $saleClass->query("select * from vw_cancel where (sdate between '$start' and '$end' ) and cancel_request like '$cashier1' order by id desc");
    }else{
        $sales = $saleClass->query("select * from vw_cancel where (sdate between '$start' and '$end' ) order by id desc");
    }
}
